  ...
  (function () {
    ...
    formSendMessage.addEventListener('submit', e => {
      ...
      if (messageInput.value) {
        let input_receiver_id = document.querySelector('#input_receiver_id');
        createChatRecord(input_receiver_id.value, messageInput.value);
        // Create a div and add a class
        // Create list item
        let li = document.createElement('li');
        li.className = 'chat-message mb-4 chat-message-right'; // Adding the required class

        // Create chat wrapper
        let chatWrapper = document.createElement('div');
        chatWrapper.className = 'd-flex overflow-hidden';

        // Create message container
        let messageContainer = document.createElement('div');
        messageContainer.className = 'chat-message-wrapper flex-grow-1';
        ...
        // Get current time
        let { hours, minutes, amPm } = getCurrentTime();

        let timeContainer = document.createElement('div');
        timeContainer.className = 'text-muted mt-1 text-end'; // Align time to the right
        timeContainer.innerHTML = `<small>${hours}:${minutes} ${amPm}</small>`;

        // Append message and time to message container
        messageContainer.appendChild(renderMsg);
        ...
        // Append to UL
        document.querySelector('ul.chat-history').appendChild(li);
    ...
    function getCurrentTime() {
      ...
      // Ensure two-digit minutes format
    ...
    // send message in db
    function createChatRecord(receiver_id, message) {
      $.ajax({
        url: formSendMessage.getAttribute('action'), // Get action URL from form
        type: 'POST',
        data: {
        ...
        error: function (xhr) {
          console.log('Error sending message:', xhr.responseJSON);
        }
      });
    }
